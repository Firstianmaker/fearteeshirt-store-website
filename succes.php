...
if (isset($_GET["orderId"])) {
  ...
  $orderQuery = "SELECT orders.alamat_pengiriman, orders.hargaTotal, payment_method.name as payment_name, payment_method.gambar
                  FROM orders 
                  INNER JOIN payment_method ON orders.payment_id = payment_method.id
                  WHERE orders.id = {$_GET["orderId"]} ";
  $order = query($orderQuery)[0];

  $detailOrderQuery = "SELECT detail_order.jumlah, products.nama, products.gambar, products.harga, size.size 
                        FROM detail_order
                        ...
                        WHERE detail_order.order_id = {$_GET["orderId"]};
                        ";
...
<body>
  <main class="relative">
    <div class="h-80 overflow-hidden lg:absolute lg:h-full lg:w-1/2 lg:pr-4 xl:pr-12">
      <img src="img/mockup/orang5.jpg" alt="TODO" class="h-full w-full object-cover object-center" />
    </div>
      ...
      <div class="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:grid lg:max-w-7xl lg:grid-cols-2 lg:gap-x-8 lg:px-8 lg:py-32 xl:gap-x-24">
        <div class="lg:col-start-2">
          ...
          <ul role="list" class="mt-6 divide-y divide-gray-200 border-t border-gray-200 text-sm font-medium text-gray-500">
            <?php
            $subTotal = 0;
            foreach ($detailOrders as $product) :
              $subTotal += $product["harga"] * $product["jumlah"];
            ?>
              <li class="flex space-x-6 py-6">
                <img src="img/baju part2/<?= $product["gambar"] ?>" alt="Model wearing men&#039;s charcoal basic tee in large." class="h-24 w-24 flex-none rounded-md bg-gray-100 object-cover object-center" />
                <div class="flex-auto space-y-1">
                  <h3 class="text-gray-900">
                    <a href="#"><?= $product["nama"] ?></a>
                  </h3>
                  <p><?= $product["size"] ?> x <?= $product["jumlah"] ?></p>
                </div>
                <p class="flex-none font-medium text-gray-900"><?= formatRupiah($product["harga"] * $product["jumlah"]) ?></p>
              </li>
            <?php endforeach ?>
          ...
          <dl class="space-y-6 border-t border-gray-200 pt-6 text-sm font-medium text-gray-500">
            <div class="flex justify-between">
              <dt>Subtotal</dt>
              <dd class="text-gray-900"><?= formatRupiah($subTotal) ?></dd>
            </div>

            <div class="flex justify-between">
              <dt>Taxes</dt>
              <dd class="text-gray-900"><?= formatRupiah($subTotal * 0.11)  ?></dd>
            </div>

            <div class="flex items-center justify-between border-t border-gray-200 pt-6 text-gray-900">
              <dt class="text-base">Total</dt>
              <dd class="text-base"><?= formatRupiah($order["hargaTotal"]) ?></dd>
            </div>
          </dl>
            ...
            <div>
              ...
              <dd class="mt-2 space-y-2 sm:flex sm:space-x-4 sm:space-y-0">
                <div class="flex align-items justify-content-center">
                  <img src="img/logopayment/<?= $order["gambar"] ?>" alt="" width="64px">
