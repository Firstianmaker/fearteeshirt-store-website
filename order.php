...
$orderQuery = "
    SELECT orders.id, orders.alamat_pengiriman, orders.hargaTotal, orders.status
    FROM orders 
...
<body>
  <div class="bg-white">
    <div class="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8 lg:pb-36">
      <div>
        <h1 class="text-2xl font-bold tracking-tight text-gray-900 sm:text-3xl">Order history</h1>
        <p class="mt-2 text-sm text-gray-500">
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Labore,
          ipsum quo! Hic nisi possimus odit consequatur accusantium in rerum
          ducimus dolores, corporis necessitatibus repudiandae assumenda
          blanditiis quibusdam magni. Eos, labore.
        </p>
      </div>
      ...
      <div class="mt-16">
        ...
        <table class="mt-4 w-full text-gray-500 sm:mt-6">
          ...
          <thead class="sr-only text-left text-sm text-gray-500 sm:not-sr-only">
            <tr>
              <th scope="col" class="py-3 pr-8 font-normal sm:w-2/5 lg:w-1/3">OrderBy</th>
              <th scope="col" class="hidden w-1/5 py-3 pr-8 font-normal sm:table-cell">Total Belanja</th>
              ...
              <tr>
                <td class="py-6 pr-8">
                  <div class="flex items-center">
                    <img src="img/baju part2/Beast Hunt Mockup (2).jpg" alt="Order Image" class="mr-6 h-16 w-16 rounded object-cover object-center">
                    <div>
                      <div class="font-medium text-gray-900"><?= $user["username"] ?></div>
                    </div>
                  </div>
                </td>
                <td class="hidden py-6 pr-8 sm:table-cell"><?= formatRupiah($order["hargaTotal"]) ?></td>
                <td class="hidden py-6 pr-8 sm:table-cell"><?= $order["id"] ?></td>
                <td class="hidden py-6 pr-8 sm:table-cell"><?= $order["status"] ?></td>
                <td class="whitespace-nowrap py-6 text-right font-medium">
                  <form action="detailOrder.php" method="post" class="inline">
                    <input type="hidden" name="orderId" value="<?= $order['id'] ?>">
                    <button type="submit" class="text-indigo-600">View Order<span class="hidden lg:inline"> Details</span><span class="sr-only">, Order #<?= $order['id'] ?></span></button>
                  </form>
                </td>
              </tr>
            <?php endforeach; ?>
