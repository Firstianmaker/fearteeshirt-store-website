...
// Memastikan keranjang belanja ada dan tidak kosong
    ...
    <div class="mx-auto max-w-5xl justify-center px-6 md:flex md:space-x-6 xl:px-0">
        ...
        <?php if (!$isCartEmpty) : ?>
          ...
          ?>
            <div class="justify-between mb-6 rounded-lg bg-white p-6 shadow-md sm:flex sm:justify-start">
              <img src="img/baju part2/<?= $product["gambar"] ?>" alt="product-image" class="w-full rounded-lg sm:w-40" />
              <div class="sm:ml-4 sm:flex sm:w-full sm:justify-between">
                <div class="mt-5 sm:mt-0">
                  <h2 class="text-lg font-bold text-gray-900">
                    <?= $product["nama"] ?>
                  </h2>
                  <p class="mt-1 text-xs text-gray-700"><?= $product["size"] ?></p>
                </div>
                <div class="mt-4 flex justify-between sm:space-y-6 sm:mt-0 sm:block sm:space-x-6">
                  <div class="flex items-center justify-end border-gray-100">
                    <a class="cursor-pointer rounded-l bg-gray-100 py-1 px-3.5 duration-100 hover:bg-blue-500 hover:text-blue-50" href="updateCart.php?update=kurang&productSizeId=<?= $product["id"] ?>">
                      -
                    </a>
                    <input class="h-8 w-8 border bg-white text-center text-xs outline-none" type="number" min="1" value="<?= $_SESSION["cart"][$product["id"]]["quantity"] ?>" />
                    <a class="cursor-pointer rounded-r bg-gray-100 py-1 px-3 duration-100 hover:bg-blue-500 hover:text-blue-50" href="updateCart.php?update=tambah&productSizeId=<?= $product["id"] ?>">
                      +
                    </a>
                  </div>
                  <div class="flex items-center space-x-4">
                    <p class="text-sm"><?= formatRupiah($product["harga"] * $_SESSION["cart"][$product["id"]]["quantity"]) ?></p>
                    <a href="updateCart.php?update=hapus&productSizeId=<?= $product["id"] ?>">
      ...
      <?php if (!$isCartEmpty) : ?>
        <div class="mt-6 h-full rounded-lg border bg-white p-6 shadow-md md:mt-0 md:w-1/3">
          <div class="mb-2 flex justify-between">
            ...
            <p class="text-gray-700"><?= formatRupiah($priceTotal)  ?></p>
          </div>
          ...
          <div class="flex justify-between">
            ...
            <div class="">
              <p class="mb-1 text-lg font-bold"><?= formatRupiah($priceTotal + 15000) ?></p>
              <p class="text-sm text-gray-700">including VAT</p>
            </div>
          </div>
          <a href="checkout.php">
            <a class="mt-6 w-full rounded-md bg-blue-500 py-1.5 font-medium text-blue-50 hover:bg-blue-600 text-center" href="checkout.php?from=cart" style="display: block;">
              Check out
            </a>
          </a>
        </div>
      <?php else : ?>
        <div class="mt-6 h-full rounded-lg border bg-white p-6 shadow-md md:mt-0 md:w-1/3">
          ...
          <a>
            <button class="mt-6 w-full rounded-md bg-blue-300 py-1.5 font-medium text-blue-50 ">
              Check out
            </button>
          </a>
        </div>
